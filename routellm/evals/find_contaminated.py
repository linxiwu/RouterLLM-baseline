...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--benchmark",
        type=str,
        choices=["mt-bench", "mmlu", "gsm8k","arc", "math", "openbookqa"],
    )
    parser.add_argument(
        "--output",
        type=str,
    ...
    if args.benchmark == "mt-bench":
        ...
        eval_prompts = pd.read_json(f"{current_dir}/arc/arc_responses.jsonl", lines=True)[
            "question_stem"
        ].tolist()  # ARC 的问题字段为 question_stem
    elif args.benchmark == "math":
        eval_prompts = pd.read_json(f"{current_dir}/math/math_responses.jsonl", lines=True)[
            "problem"
        ].tolist()  # Math 的问题字段为 problem
    elif args.benchmark == "openbookqa":
        eval_prompts = pd.read_json(
            f"{current_dir}/openbookqa/openbookqa_responses.jsonl", lines=True
        )["question_stem"].tolist()  # OpenBookQA 的问题字段为 question_stem
